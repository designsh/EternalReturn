...
public:
	...
	void Read(GameEngineFile* _File)
	{
		...
		{
			if (path.filename().string() == "EternalReturn")
			{
				break;
			}
		...
		std::string prevPath;
		std::string FindPath = "C:\\Project\\FoxTeam";

		if (false == GameEngineDirectory::IsExist(DifTexturePath) &&
			"" != DifTexturePath)
		{
			prevPath = DifTexturePath;
			prevPath.erase(DifTexturePath.find(FindPath) + FindPath.size());
			...
			"" != NorTexturePath)
		{
			...
			prevPath.erase(NorTexturePath.find(FindPath) + FindPath.size());
			...
			"" != SpcTexturePath)
		{
			...
			prevPath.erase(SpcTexturePath.find(FindPath) + FindPath.size());
